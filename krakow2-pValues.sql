...
) AS x ON g.rok = x.rok AND g.gmina = x.gmina AND g.lista = x.lista SET g.expectS = x.s WHERE g.rok = 9999;
...
SET g.pExact = sd.prob, g.pLess = sd.cdf, g.pMore = 1 - sd.cdf + sd.prob, g.pValue = LEAST(sd.cdf, 1 - sd.cdf + sd.prob) WHERE g.rok = 9999;
...
WHERE MOD(g.man, 1) != 0 AND g.rok = 9999;