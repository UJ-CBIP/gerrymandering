...
INNER JOIN gerry.gerry AS x ON g.rok = 9999 AND x.rok = 2014 AND x.v <= g.v AND x.s >= g.s AND (g.gmina != x.gmina OR g.lista != x.lista)
...
INNER JOIN gerry.gerry AS x ON g.rok = 9999 AND x.rok = 2014 AND x.normV <= g.normV AND x.s >= g.s AND (g.gmina != x.gmina OR g.lista != x.lista) AND x.normV > 0
WHERE g.normV > 0
...
UPDATE gerry.gerryHypoSV SET up = 0, u = 0 WHERE normV = 0 AND s > 0;
