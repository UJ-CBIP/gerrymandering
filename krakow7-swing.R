...
wyb0 <- filter(wyb, listOkr > 1, rok == 9999) %>% select(rok, gmina, okreg, lista, listOkr, pct, reqSwing)
wyb0$ecdfV <- pmap_dbl(transmute(wyb0, r = 2014, n = listOkr, v = pct),
   function(r, n, v) pkde1d(v, ddf$kde[ddf$r == r & ddf$n == n][[1]])
)
wyb0$reqSwingQ <- pmap_dbl(transmute(wyb0, r = 2014, n = listOkr, v = pct + reqSwing),
   function(r, n, v) pkde1d(v, ddf$kde[ddf$r == r & ddf$n == n][[1]])
)
